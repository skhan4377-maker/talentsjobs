...
class CareerServices extends CI_Controller {
	...
	public function index() {		
		...
		$this->load->view('website/services/career_services', $services);
    ...
    private function build_service_html($service) {
		...
		foreach ($service['features'] as $feature) {
			...
			// Normalize values safely (convert to float if numeric, else keep 'N/A')
			$plan_mrp      = is_numeric($plan_mrp) ? (intval($plan_mrp) == $plan_mrp ? intval($plan_mrp) : number_format((float)$plan_mrp, 2)) : 'N/A';
			$plan_total    = is_numeric($plan_total) ? (intval($plan_total) == $plan_total ? intval($plan_total) : number_format((float)$plan_total, 2)) : 'N/A';
			$plan_discount = is_numeric($plan_discount) ? (intval($plan_discount) == $plan_discount ? intval($plan_discount) : number_format((float)$plan_discount, 2)) : '0';
    ...
    public function view($slug) {
	   ...
	   // Sanitize and validate input slug
        ...
        if (!empty($service)) {
            ...
            $this->load->view('website/services/list_all_services', $data);
        } else {
            ...
            if (!empty($feature)) {
                ...
                $profile_features = $this->CareerService_model->get_profile_features_by_feature_id($feature['feature_id']);
                $headers = $this->CareerService_model->get_advantage_headings_by_feature_id($feature['feature_id']);
    
				// Fetch table rows
				$rows = $this->CareerService_model->get_feature_advantages_by_feature_id($feature['feature_id']);
				$faqs = $this->CareerService_model->get_faqs_by_feature_id($feature['feature_id']);
    
				
				  // Send to view
					$data['feature'] = $feature;	
					$data['profile_features'] = $profile_features;					
					$data['advantage_headings_columns'] = $headers;
					$data['advantage_headings'] = $rows;				
					$data['faqs'] = $faqs;
								
				
                // Load the feature view with all the necessary data
                $this->load->view('website/services/feature_detail', $data);
    ...
    public function get_plan_durations($feature_slug) {
		...
		// Optional query parameters for selected experience level and plan index
		$selectedExpIndex  = $this->input->get('exp') !== null ? (int)$this->input->get('exp') : null;
		$selectedPlanIndex = $this->input->get('plan') !== null ? (int)$this->input->get('plan') : null;

		$feature = $this->CareerService_model->get_feature_by_slug($feature_slug);
		
		if (!empty($feature)) {
        $plan_durations = $this->CareerService_model->get_plan_durations_by_feature_id($feature['feature_id']);
        $feature_custom_label = $feature['feature_custom_label'];
		
		
        if (!empty($plan_durations)) {
            // Group plan durations by experience level
            $experienceLevels = [];
            foreach ($plan_durations as $plan) {
                $experienceLevel = $plan['experience_level'];

                // Skip hidden experience levels
                //if ($plan['experience_level_visibility'] !== '1') {				
                    //continue;
                //}
				
                // If not set, initialize group
                if (!isset($experienceLevels[$experienceLevel])) {
                    $experienceLevels[$experienceLevel] = [
                        'experience_level' => $experienceLevel,
                        'flag_active'      => '0', // default value, will override below
                        'plan_level'            => []
                    ];
                }
            }

            // Now fetch the plan details for each experience level
            foreach ($experienceLevels as $levelKey => $info) {
                $plan_level = $this->CareerService_model->get_plan_details_by_experience($levelKey, $feature['feature_id']);
                $experienceLevels[$levelKey]['plan_level'] = $plan_level;
            }

            // Convert associative array to numeric array
            $experienceLevels = array_values($experienceLevels);

            // Determine default experience index if none is provided
            if ($selectedExpIndex === null) {
                $selectedExpIndex = 0;
                foreach ($experienceLevels as $i => $exp) {
                    // If any experience has a plan with flag_active true, choose that.
                    foreach ($exp['plan_level'] as $plan) {
                        if ($plan['flag_active'] === '1') {
                            $selectedExpIndex = $i;
                            break 2;
                        }
                    }
            ...
            // Mark the selected experience level active
            foreach ($experienceLevels as $i => &$exp) {
                $exp['flag_active'] = ($i === $selectedExpIndex) ? '1' : '0';
            }
            unset($exp); // break reference

            $selectedExp = $experienceLevels[$selectedExpIndex];

            // Determine default plan index if none is provided
            if ($selectedPlanIndex === null) {
                $selectedPlanIndex = 0;
                foreach ($selectedExp['plan_level'] as $i => $plan) {
                    if ($plan['flag_active'] === '1') {
                        $selectedPlanIndex = $i;
                        break;
                    }
            ...
            // Update plans to mark the selected plan active
            foreach ($selectedExp['plan_level'] as $i => &$plan) {
                $plan['flag_active'] = ($i === $selectedPlanIndex) ? '1' : '0';
            }
            unset($plan);
		   		  
 
            // 1. Experience Level Buttons
			$expHtml = '<div class="grid grid-cols-2 gap-4 mb-6">';
			foreach ($experienceLevels as $index => $exp) {
				if (!trim($exp['experience_level'])) {
					continue;
				}
				// Check the visibility flag from the first plan
				if (empty($exp['plan_level']) || $exp['plan_level'][0]['experience_level_visibility'] !== '1') {
					continue; // Skip if not visible
				}
				$activeClass = ($exp['flag_active'] === '1') ? 'border-b-2 border-blue-500' : '';
				$expHtml .= '<button class="exp-toggle py-2 text-center font-medium ' . $activeClass . '" data-exp-index="' . $index . '">Exp: ' . htmlspecialchars($exp['experience_level']) . '</button>';
			}
			$expHtml .= '</div>';

			
            // 2. Plan Durations for Selected Experience Level
            $planHtml = '<div class="text-center mb-6">';
            $planHtml .= '<h3 class="font-medium mb-2">' . htmlspecialchars($feature_custom_label) . '</h3>';
            $planHtml .= '<div class="inline-flex items-center border border-gray-300 rounded-full overflow-hidden">';
            foreach ($selectedExp['plan_level'] as $index => $plan) {
                $planDurationLabel = $plan['plan_duration'];
				if($plan['plan_duration_visibility'] !=='1'){
					continue; // Skip if not visible
				}
                $activeClass = ($plan['flag_active'] === '1') ? 'bg-purple-100 text-purple-600 border border-purple-300' : 'text-gray-600 hover:bg-gray-100';
                $planHtml .= '<button class="plan-toggle px-4 py-2 focus:outline-none ' . $activeClass . '" data-plan-index="' . $index . '">' . htmlspecialchars($planDurationLabel) . '</button>';
            }
            $planHtml .= '</div></div>';

            // 3. Pricing Details from Selected Plan
            $selectedPlan = $selectedExp['plan_level'][$selectedPlanIndex] ?? null;
			$pricingHtml = '';
			if ($selectedPlan) {
				$pricingHtml = '<div class="text-center mb-6">';
				if ($selectedPlan['plan_discount'] > 0) {
					$pricingHtml .= '<div id="strikePrice" class="text-gray-500 line-through">₹' . $selectedPlan['plan_mrp'] . '</div>';
				}
				$pricingHtml .= '<div id="activePrice" class="text-3xl font-bold my-2">₹' . round($selectedPlan['plan_total'],0) . '</div>';
				$pricingHtml .= '<div id="monthlyCost" class="text-gray-600">₹' . round($selectedPlan['monthly_cost'],0) . '/month</div>';
				$pricingHtml .= '</div>';
			}
			
			// 4. CTA & Additional Info (static for this example)
            // Build dynamic price breakup based on selected plan data.
			$priceBreakupHtml = '';
			if ($selectedPlan) {
				// Retrieve values from the selected plan.
				$plan_mrp                = floatval($selectedPlan['plan_mrp']);
				$plan_discount           = floatval($selectedPlan['plan_discount']); // e.g., 16.35 for 16.35%
				$feature_coupon_discount = isset($selectedPlan['feature_coupon_discount']) ? floatval($selectedPlan['feature_coupon_discount']) : 0;
				// Use plan_taxes as a percentage directly (e.g., 18 for 18%)
				$plan_taxes              = floatval($selectedPlan['plan_taxes'] * 100);

				// Calculate the discount amount from the total MRP.
				$discount_amount = $plan_mrp * ($plan_discount / 100);

				// Calculate the base amount after discount and coupon discount.
				$base_amount = $plan_mrp - $discount_amount - $feature_coupon_discount;
				
				// Calculate the GST amount on the base amount.
				$gst_amount = $base_amount * ($plan_taxes / 100);

				// Calculate the total price.
				// Total Price = (MRP - discount - coupon discount) + GST
				$total_price = $base_amount + $gst_amount;

				// Build the dynamic price breakup HTML.
				$priceBreakupHtml .= '<div id="priceBreakup" class="hidden mt-4 p-4 border rounded-lg bg-gray-100">';
				$priceBreakupHtml .= '<p class="text-sm text-gray-600">';
				$priceBreakupHtml .= 'Total MRP: ₹' . number_format($plan_mrp) . '<br />';
				$priceBreakupHtml .= 'Discount: -₹' . number_format($discount_amount) . '<br />';
				$priceBreakupHtml .= 'Coupon Discount: -₹' . number_format($feature_coupon_discount) . '<br />';
				$priceBreakupHtml .= 'GST: ₹' . number_format($gst_amount) . '<br />';
				$priceBreakupHtml .= 'Total Price: ₹' . number_format($total_price);
				$priceBreakupHtml .= '</p></div>';
			}
						
            $ctaHtml = '<div class="grid grid-cols-2 gap-4 mb-6">
				<!-- Add to Cart Button -->
				<button class="p-2 border rounded-lg flex items-center justify-center add-to-cart-btn" 
					data-duration-id="' . $selectedPlan['duration_id'] . '" 
					data-feature-id="' . $selectedPlan['feature_id'] . '">
				...
				<!-- Buy Now Button -->
				<button class="p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 buy-now-btn" 
					data-duration-id="' . $selectedPlan['duration_id'] . '" 
					data-feature-id="' . $selectedPlan['feature_id'] . '">
				  Buy Now
				</button>
              ...
              <div class="text-center text-sm text-gray-600 mb-4">
                ...
                <button id="viewPriceBreakupButton" class="text-blue-600 hover:underline mt-1">View price breakup</button>
              </div>' . $priceBreakupHtml . '
              <div class="flex items-center justify-center gap-2 pt-4 border-t">
                <svg class="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                  <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/>
                </svg>
                <span class="text-sm text-gray-600">100% Secure Payments</span>
              </div>';

            $html = '<div class="border rounded-lg p-6 bg-white">';
            $html .= $expHtml;
            $html .= '<div id="planDurationsArea">';
            $html .= $planHtml . $pricingHtml . $ctaHtml;
            $html .= '</div></div>';
    ...
    private function generate_experience_html($plan_durations, $plan_details, $feature_custom_label) {
        $html ='';
        $html .= '<div class="order-1 md:order-2 md:col-span-1">
			<div class="border rounded-lg p-6 bg-white">
			<div class="grid grid-cols-2 gap-4 mb-6">';
			 foreach ($plan_durations as $plan) {
				$experienceId = strtolower(str_replace(' ', '-', $plan['experience_level']));
				$flag_active = $plan['flag_active'] === '1' ? 'border-b-2' : '';				
				// Check if experience level is not 0, if it's 0, skip rendering
				if ($plan['experience_level_visibility'] == '1') {	
				  $html .='<button id="exp0to8" class="exp-toggle py-2 text-center  ' . $flag_active . ' 
				  border-blue-500  font-medium">Exp: ' . htmlspecialchars($plan['experience_level']) . '</button>';
            ...
            <div class="text-center mb-6">
              <h3 class="font-medium mb-2">'.$feature_custom_label.'</h3>
			  ...
			  foreach ($plan_details as $plan) {
				...
				$flag_active = ($plan['flag_active'] === '1') ? 'bg-purple-100' : ''; // Add active class if flag_active is set			
                $html .=' <button class="' . $flag_active . ' text-purple-600 px-4 py-2 border border-purple-300 focus:outline-none">
					' . $plan['plan_duration'] . '
				  </button>';
              }
            ...
            $html .='</div> </div>';
            // Now, show plan pricing for whichever plan is active
			// (If you only want to show *one* active plan's pricing)
			foreach ($plan_details as $plan) {
				if ($plan['flag_active'] === '1') {
					$plan_mrp       = isset($plan['plan_mrp'])       ? $plan['plan_mrp']       : 0;
					$plan_total     = isset($plan['plan_total'])     ? $plan['plan_total']     : 0;
					$monthly_cost   = isset($plan['monthly_cost'])   ? $plan['monthly_cost']   : 0;

					$html .= '
					<div class="text-center mb-6">
					  <div id="strikePrice" class="text-gray-500 line-through">₹' . $plan_mrp . '</div>
					  <div id="activePrice" class="text-3xl font-bold my-2">₹' . $plan_total . '</div>
					  <div id="monthlyCost" class="text-gray-600">₹' . $monthly_cost . '/month</div>
					</div>';

					// Stop after showing the first active plan’s pricing
					break;
				}
			}
			
            $html .='<div class="grid grid-cols-2 gap-4 mb-6">
              <button class="p-2 border rounded-lg flex items-center justify-center">
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z"/>
                </svg>
              </button>
              <button class="p-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Buy Now</button>
            </div>
            <!-- Additional Info -->
            <div class="text-center text-sm text-gray-600 mb-4">
              <p>Inclusive of all taxes</p>
            ...
            <!-- Price Breakup Details (hidden by default) -->
            <div id="priceBreakup" class="hidden mt-4 p-4 border rounded-lg bg-gray-100">
              <p class="text-sm text-gray-600">
                Price includes base fare, taxes, and processing fees.
                <br />Base Price: ₹4,500, Taxes: ₹500, Fees: ₹427.
              </p>
            </div>
            <!-- Secure Payment -->
            <div class="flex items-center justify-center gap-2 pt-4 border-t">
