<!-- FILE: application/views/templates/resume_templates_slider.php -->
<?php
// No fallback – relies on controller passing $resumeTemplates
if (empty($resumeTemplates)) {
    return; // silently exit if no data
}

if (!isset($resumeToken) && $this->session->userdata('role') === 'candidate') {
    $resumeToken = $this->session->userdata('login_token');
}
$baseResumeUrl = 'https://resume.talentsjobs.in/templates';
$apiBase = base_url('api/resume-templates');
?>

<section class="bg-gradient-to-b from-white to-gray-50 py-12 md:py-16 lg:py-20 overflow-x-hidden">
    <div class="container mx-auto px-4 sm:px-6 max-w-7xl overflow-x-hidden">
        <!-- Section Header -->
        <div class="text-center mb-8 md:mb-12">
            <span class="inline-flex items-center gap-2 bg-blue-50 text-blue-700 text-sm font-semibold px-4 py-1.5 rounded-full mb-4">
                <i class="fas fa-magic text-xs"></i> Build in Minutes
            </span>
            ...
            <p class="text-gray-600 max-w-2xl mx-auto text-sm md:text-base">
                Choose from 50+ ATS-friendly, recruiter-approved designs. Download in PDF & get noticed 3x faster.
            </p>
        </div>

        <!-- Slider Wrapper -->
        <div class="relative">
            <div class="swiper resumeTemplatesSwiper overflow-visible">
                <div class="swiper-wrapper">
                    <?php foreach ($resumeTemplates as $template): 
                        ...
                        $previewUrl = $template['preview'] ?? ($template['thumbnail_url'] ?? 'https://placehold.co/400x500/e2e8f0/1e293b?text=' . urlencode($template['name']));
                        $templateUrl = $baseResumeUrl . '?template_id=' . $templateId;
                        if (!empty($resumeToken)) $templateUrl .= '&token=' . urlencode($resumeToken);
                    ?>
                    <div class="swiper-slide" data-template-id="<?= $templateId ?>">
                        <div class="group relative bg-white rounded-2xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 overflow-hidden flex flex-col h-full border border-gray-100">
                            <?php if (!empty($template['popular'])): ?>
                                <div class="absolute top-3 left-3 z-10 bg-gradient-to-r from-amber-400 to-orange-500 text-white text-xs font-bold px-2.5 py-1 rounded-full shadow-md flex items-center gap-1">
                                    <i class="fas fa-fire text-xs"></i> <span>Popular</span>
                                </div>
                            <?php endif; ?>
                            ...
                            <div class="p-4 flex flex-col flex-grow">
                                ...
                                <div class="flex items-center gap-3 text-xs text-gray-500 mb-3">
                                    <div class="flex items-center gap-1"><i class="fas fa-download text-gray-400"></i> <span><?= $downloadCount ?> downloads</span></div>
                                    <div class="flex items-center gap-1"><i class="fas fa-clock text-gray-400"></i> <span>5-min setup</span></div>
                                </div>
            ...
            <!-- Navigation Buttons (Tailwind only) -->
            <div class="swiper-button-prev resume-swiper-prev !absolute !top-1/2 !-translate-y-1/2 !w-10 !h-10 !bg-white !rounded-full !shadow-md !flex !items-center !justify-center !z-20 hover:!bg-blue-50 transition-all !left-2 md:!left-4 after:!text-base after:!font-bold after:!text-gray-700"></div>
            <div class="swiper-button-next resume-swiper-next !absolute !top-1/2 !-translate-y-1/2 !w-10 !h-10 !bg-white !rounded-full !shadow-md !flex !items-center !justify-center !z-20 hover:!bg-blue-50 transition-all !right-2 md:!right-4 after:!text-base after:!font-bold after:!text-gray-700"></div>
        </div>

        <!-- Pagination -->
        <div class="swiper-pagination resume-swiper-pagination !relative !mt-8"></div>
        ...
        <div class="mt-12 md:mt-16 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-5 shadow-sm border border-blue-100">
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <a href="<?= $baseResumeUrl . (!empty($resumeToken) ? '?token=' . urlencode($resumeToken) : '') ?>" 
                   target="_blank"
                   class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-5 py-2.5 rounded-xl shadow-md transition-all hover:shadow-lg">
...
<script>
(function() {
    // Track template usage when slide becomes visible
    function trackTemplateUsage(templateId) {
        if (!templateId) return;
        fetch('<?= $apiBase ?>/track-usage', {
            method: 'POST',
    ...
    // Track download when user clicks "Use Template" or "Build Resume Now"
    function trackDownload(templateId) {
        if (!templateId) return;
        fetch('<?= $apiBase ?>/track-download', {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: 'template_id=' + encodeURIComponent(templateId)
    ...
    // Attach click listeners to all download links
    document.querySelectorAll('.track-download').forEach(link => {
        link.addEventListener('click', function(e) {
            const tid = this.getAttribute('data-template-id');
            if (tid) trackDownload(tid);
        });
    });

    function initResumeSlider() {
        if (typeof Swiper === 'undefined') {
            setTimeout(initResumeSlider, 200);
            return;
        }
        const container = document.querySelector('.resumeTemplatesSwiper');
        if (!container) return;
        ...
        const swiper = new Swiper(container, {
            ...
            spaceBetween: 20,
            navigation: {
                nextEl: '.resume-swiper-next',
                prevEl: '.resume-swiper-prev',
            },
            pagination: {
            ...
            on: {
                init: function() {
                    document.querySelectorAll('.swiper-slide-visible').forEach(slide => {
                        const tid = slide.getAttribute('data-template-id');
                        if (tid) trackTemplateUsage(tid);
                    });
                },
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', initResumeSlider);
    } else {
        initResumeSlider();
    }
})();
