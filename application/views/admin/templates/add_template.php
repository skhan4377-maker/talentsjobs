  ...
  <div class="max-w-6xl mx-auto space-y-8">
    ...
    <div id="resume_template_form_wrapper" class="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-200">
      <div class="flex flex-col sm:flex-row sm:items-center justify-between border-b pb-4 mb-6 gap-4">
        <div class="flex-1">
          <h3 id="form_heading" class="text-xl md:text-2xl font-bold text-gray-800">
            Add Resume Template
          </h3>
          <p class="text-sm text-gray-600 mt-1">Create and manage professional resume templates</p>
        </div>
      ...
      <form id="form_resume_template" enctype="multipart/form-data" method="post" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div class="col-span-1">
              <label class="block text-sm font-medium text-gray-700 mb-2">
                Select Feature <span class="text-red-500">*</span>
              </label>
              <select id="feature_select" name="feature_id" 
                ...
                <?php foreach ($features as $f): ?>
                  <option value="<?= $f['feature_id'] ?>"><?= $f['feature_name'] ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            ...
            <div class="col-span-1">
              ...
              <input type="number" name="template_version" value="1" min="1"
                     class="w-full border-gray-300 rounded-lg shadow-sm px-3 py-2 text-sm md:text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition">
            </div>
        ...
        <div class="bg-purple-50 p-3 md:p-4 rounded-lg">
          ...
          <div id="advanced_config_section" class="hidden space-y-4 transition-all duration-300">
            <!-- Layout Config JSON -->
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-2">
              ...
              <textarea name="layout_config" rows="5"
                        ...
                        placeholder='{"section_order": ["header", "experience", "education"], "font_family": "Arial", "color_scheme": "blue"}'>{
  "columns": 2,
  "gap": "24px",
  "widths": {
    "primary": "35%",
      ...
      "certifications"
            ...
            <div>
              ...
              <textarea name="schema_json" rows="5"
                        class="w-full border-gray-300 rounded-lg shadow-sm px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition"
                        placeholder='{"@context": "https://schema.org/", "@type": "ResumeTemplate", "name": "Template Name"}'>{
    ...
    "hobbies": "array"
            ...
            <div>
              ...
              <textarea name="zones_supported" rows="5"
                        class="w-full border-gray-300 rounded-lg shadow-sm px-3 py-2 text-sm font-mono focus:ring-2 focus:ring-purple-500 focus:border-purple-500 transition"
                        placeholder='["header", "summary", "experience", "education", "skills", "projects"]'>[
  "header",
  "summary",
  "experience",
  "education",
  ...
  "hobbies"
        ...
        <div class="bg-indigo-50 p-3 md:p-4 rounded-lg">
          ...
          <textarea id="html_layout" name="html_layout" rows="8"
                    ...
                    placeholder="Enter your HTML template code here..."></textarea>
...
class ResumeTemplateManager {
    static showNotification(message, type = 'success') {
        ...
        formResponse.innerHTML = `
            <div class="flex items-start">
                ...
                <button type="button" class="ml-auto -mx-1.5 -my-1.5 rounded-lg p-1.5 inline-flex h-8 w-8 hover:bg-white hover:bg-opacity-50 transition" onclick="ResumeTemplateManager.hideNotification()">
    ...
    static resetForm() {
        const form = document.getElementById('form_resume_template');
        if (form) {
            form.reset();
            if (tinymce.get('html_layout')) tinymce.get('html_layout').setContent('');
            this.showNotification('', 'info');
            const fileInput = form.querySelector('input[type="file"]');
            if (fileInput) fileInput.value = '';
            const advancedSection = document.getElementById('advanced_config_section');
            if (advancedSection) advancedSection.classList.add('hidden');
            const toggleIcon = document.getElementById('advanced_toggle_icon');
    ...
    static validateForm() {
        ...
        if (!featureSelect.value) {
            this.showNotification('Please select a feature', 'error');
            featureSelect.focus();
            return false;
        }
...
function previewTemplate() {
    let htmlContent = tinymce.get('html_layout') ? tinymce.get('html_layout').getContent() : document.getElementById('html_layout').value;
    if (htmlContent.trim()) {
        const previewWindow = window.open('', '_blank');
        previewWindow.document.write(`
            <!DOCTYPE html>
            <html>
            <head><title>Template Preview</title>
            <style>
            ...
            <body><div class="template-preview">${htmlContent.replace(/\{\{(\w+)\}\}/g, '<span class="placeholder">[$1]</span>')}</div></body>
            </html>
        `);
        previewWindow.document.close();
    } else {
        ResumeTemplateManager.showNotification('Please enter HTML content to preview', 'warning');
    }
}

function clearForm() {
    if (confirm('Are you sure you want to clear all form data?')) {
        ResumeTemplateManager.resetForm();
        ResumeTemplateManager.showNotification('Form cleared successfully', 'info');
    }
}
...
$(document).ready(function () {
    ...
    // Feature select feedback
    $('#feature_select').on('change', function () {
        const featureId = $(this).val();
        if (featureId) ResumeTemplateManager.showNotification('Loading feature details...', 'info');
    });

    // Form submission with CSRF using global functions
    $('#form_resume_template').on('submit', function (e) {
        e.preventDefault();
        if (!ResumeTemplateManager.validateForm()) return;

        if (tinymce.get('html_layout')) tinymce.triggerSave();

        ResumeTemplateManager.toggleLoading(true);
        ResumeTemplateManager.showNotification('Saving template...', 'info');

        const formData = new FormData(this);
        // Append CSRF token using global functions
        ...
        try {
            ...
            ResumeTemplateManager.toggleLoading(false);
            ResumeTemplateManager.showNotification('Invalid JSON in advanced configuration fields', 'error');
            return;
        }

        $.ajax({
            url: '<?= base_url("admin/features/ResumeTemplates/save") ?>',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            ...
            success: function (response) {
                ...
                if (response.csrf_token) {
                    const newName = response.csrf_name || getCSRFName();
                    updateCSRFToken(response.csrf_token, newName);
                }
                ResumeTemplateManager.toggleLoading(false);

                if (response.success) {
                    ResumeTemplateManager.showNotification(response.message || 'Template saved successfully!', 'success');
                    setTimeout(() => ResumeTemplateManager.resetForm(), 2000);
                } else {
                    ResumeTemplateManager.showNotification(response.message || 'Failed to save template. Please try again.', 'error');
                }
            },
            error: function (xhr, status, error) {
                ...
                else if (xhr.status === 413) errorMessage = 'File size too large. Please choose a smaller image.';
                else if (xhr.status === 415) errorMessage = 'Unsupported file type. Please use JPEG, PNG, GIF, or WebP images.';
                ResumeTemplateManager.showNotification(errorMessage, 'error');
    ...
    $('input[type="file"]').on('change', function(e) {
        ...
        if (file) {
            if (file.size > 5 * 1024 * 1024) {
                ResumeTemplateManager.showNotification('File size must be less than 5MB', 'error');
                $(this).val('');
                return;
            }
            const validTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
            if (!validTypes.includes(file.type)) {
                ResumeTemplateManager.showNotification('Please select a valid image file (JPEG, PNG, GIF, WebP)', 'error');
                $(this).val('');
                return;
            }
            ...
            ResumeTemplateManager.showNotification('Image selected successfully', 'success');
        }
    });
